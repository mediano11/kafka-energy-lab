...
- **`anomaly_type`**: Тип аномалії, який описує виявлене відхилення в роботі активу.

  #### **`forecast_accuracy`**

  Розбіжність між прогнозованим та фактичним значенням вихідної потужності.

  - **Критерій:** значний відсоток похибки між `forecasted_output` та фактичним `power_output`
  - **Severity:**
    - `medium`, якщо похибка < 50%
    - `high`, якщо похибка ≥ 50%
  - **Опис:** "Велика розбіжність між прогнозом та фактичним значенням"

  ***

  #### **`negative_capacity`**

  Виявлена негативна доступна потужність, що свідчить про помилкові дані або несправність.

  - **Критерій:** `available_capacity < 0`
  - **Severity:** `critical`
  - **Опис:** "Негативна доступна потужність"

  ***

  #### **`low_soc`** _(для батарей)_

  Критично низький рівень заряду батареї (State of Charge).

  - **Критерій:** `soc < 5%`
  - **Severity:** `high`
  - **Опис:** "Критично низький рівень заряду"

  ***

  #### **`low_fuel`** _(для дизель-генераторів)_

  Низький рівень палива, що може призвести до зупинки або нестабільної роботи генератора.

  - **Критерій:** `fuel_level < 10%`
  - **Severity:** `high`
  - **Опис:** "Критично низький рівень палива"
